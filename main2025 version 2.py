...
# Function to handle the bobbing motion in a new thread
def bobbing(squid, bobbingDuration):
    endTime = ticks_ms() + (bobbingDuration * 1000)
    while ticks_ms() < endTime:
        squid.surface(seconds=1)
        squid.sink(seconds=1)
...
def main():
    ...
    for i in range(numProfiles):
        ...
        if currentDepth >= 2.5:
            ...
            # Start the bobbing motion in a new thread
            start_new_thread(bobbing, (squid, bobbingDuration))
