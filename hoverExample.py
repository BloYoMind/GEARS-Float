...
def pressureToDepth(pressure):
    return round((pressure - 101.325) / 9.78, 2)
...
class SquidControl():
    ...
    # Blink the light a specified number times at a specified speed
    ...
    def hoverThread(self, depthTarget, seconds):
        def action(depthTarget, seconds):
            hoverStart = ticks_ms()
            while (ticks_ms() - hoverStart) <= seconds:
                currentDepth = pressureToDepth(self.getPressure())
                if currentDepth > depthTarget:
                    self.sinkThread(1)
                elif currentDepth < depthTarget:
                    self.surfaceThread(1)
        start_new_thread(action, (depthTarget, seconds))
    ...
    def record(self, direction):
        ...
        for i in range(8):
            ...
            dataPoint.append(str(pressureToDepth(currentPressure))) # Depth calculation
...
def main():
            ...
            with open('row.html','r') as r:
                with open('profileEnd.html','r') as pE:
                    with open('foot.html','r') as f:
                        head = h.read()
                        print("Loaded file 'head.html.'")
                        profileStart = pS.read()
                        print("Loaded file 'profileStart.html.'")
                        row = r.read()
                        print("Loaded file 'row.html.'")
                        profileEnd = pE.read()
                        print("Loaded file 'profileEnd.html'")
                        foot = f.read()
                        print("Loaded file 'foot.html.'")
                        print('All HTML files loaded.')
    ...
    for i in range(numProfiles):
        ...
        # Add the new data to the list of datas
